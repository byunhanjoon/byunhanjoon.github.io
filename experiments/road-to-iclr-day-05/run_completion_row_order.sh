...
output_dir="results/completion_row_order"
log_dir="results/completion_row_order_logs"
mkdir -p "$output_dir" "$log_dir"

datasets=(australian_credit_approval bank_marketing_subscription fremtpl_claim_count kdd17_stock_return)
...
for dataset in "${datasets[@]}"; do
  for model in "${models[@]}"; do
    ...
    stem="${dataset}__${model}__row_order"
    ...
    CUDA_VISIBLE_DEVICES="$gpu" OPENBLAS_NUM_THREADS=1 OMP_NUM_THREADS=1 MKL_NUM_THREADS=1 \
      "$python_bin" completion_row_order.py --dataset "$dataset" --model "$model" \
