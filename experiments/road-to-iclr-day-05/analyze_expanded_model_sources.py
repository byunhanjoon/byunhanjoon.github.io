"""Recompute clustered packing effects after adding HistGB and CatBoost."""
...
def main() -> None:
    original_effects = pd.read_csv(RESULTS / "combined_packing_source_effects.csv")
    late = pd.concat([
        pd.read_csv(RESULTS / "late_source_packing_calibration.csv"),
        pd.read_csv(RESULTS / "late_source_b_packing_calibration.csv"),
        pd.read_csv(RESULTS / "modern_model_packing_calibration.csv"),
    ], ignore_index=True)
    late_sources = set(late.dataset)
    rows, summaries = [], {}
    for name, (family, action, control) in COMPARISONS.items():
        current = late[
            (late.family == family) & late.method.isin((action, control))
        ]
        means = current.groupby(["dataset", "method"]).score_rmse.mean().unstack()
        means["percent_reduction"] = 100 * (1 - means[action] / means[control])
        retained = original_effects[
            (original_effects.comparison == name)
            & ~original_effects.source.isin(late_sources)
        ][["source", "percent_reduction"]]
        appended = means.reset_index().rename(columns={"dataset": "source"})[
            ["source", "percent_reduction"]
        ]
        effects = pd.concat([retained, appended], ignore_index=True).sort_values("source")
        values = effects.percent_reduction.to_numpy(dtype=float)
        rng = np.random.default_rng(RMS.stable_seed("expanded-model-source", name))
        ...
        for row in effects.itertuples(index=False):
            rows.append({
                "comparison": name, "source": row.source,
    ...
    pd.DataFrame(rows).to_csv(
        RESULTS / "expanded_model_source_effects.csv", index=False
    )
    summary = {
        "status": "complete", "evidence_status": "post_outcome_sensitivity",
        "late_source_models": 5, "unique_sources": 11,
    ...
    (RESULTS / "expanded_model_source_summary.json").write_text(
